...
) TO 'euroappa-nuts-2021.gpkg'
  WITH (FORMAT gdal, DRIVER 'GPKG', SRS 'EPSG:4326', OVERWRITE true);
  
COPY ( 
  SELECT * EXCLUDE(geom) 
  FROM 'euroappa-nuts-2021.gpkg' 
) TO 'euroappa-nuts-2021.parquet' 
  WITH (OVERWRITE true);

COPY ( 
  SELECT * EXCLUDE(geom) 
  FROM 'euroappa-nuts-2021.gpkg' 
) TO 'euroappa-nuts-2021.csv.gz' 
  WITH (OVERWRITE true);  
...
COPY (
 ...
 FROM interactions
  ...
  ON ST_Within(interactions.location, countries.country)
) TO 'euroappa-cntr-2024.gpkg'
  WITH (FORMAT gdal, DRIVER 'GPKG', SRS 'EPSG:4326', OVERWRITE true);


COPY ( 
  SELECT * EXCLUDE(geom) 
  FROM 'euroappa-cntr-2024.gpkg' 
) TO 'euroappa-cntr-2024.parquet' 
  WITH (OVERWRITE true);

COPY ( 
  SELECT * EXCLUDE(geom) 
  FROM 'euroappa-cntr-2024.gpkg' 
) TO 'euroappa-cntr-2024.csv.gz' 
  WITH (OVERWRITE true);  
